...
namespace ClienteWeb.Pages.ReporteFactura
{
    public class IndexModel : PageModel
    {
        ...
        public async Task getr()
        {
            ...
            using (httpClient)
            {
                var data = new { username = sesion,id_factura= TempData["idf"] };

                var content = new StringContent(JsonConvert.SerializeObject(data), Encoding.UTF8, "application/json");
                var response = await httpClient.PostAsync("http://localhost:5119/api/reporte/user/Factura", content);

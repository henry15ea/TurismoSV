...
namespace webApi_Turismo.Controllers.admControllers.factura.encabezado
{
    ...
    public class getEncabezadoFController : Controller
    {
        private readonly IConfiguration _configuration;


        public getEncabezadoFController(IConfiguration configuration)
        {
        ...
        [HttpPost]
        public IActionResult getEncabezadoF(cUserModel datamodel)
        {
            bool getdata = false;
            var dataResp = new
            {
            ...
            if (datamodel != null)
            {
                ...
                encabezadoFacturaFuncs dp = new encabezadoFacturaFuncs();
                var dataDB = new List<vencabezadoExt>();
                try
                {
                    dataDB = dp.GetEncabezadoFacuturaList();
                    getdata = true;
                }
                catch
                {
                    dataDB.Add(new vencabezadoExt
                    {
                        Id_factura = "null",
                        Nombre = "null",
                        Apellido = "null",
